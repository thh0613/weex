...
FunctionsExposedByJS *ScriptBridgeInMultiSo::GetExposedFunctions() {
  FunctionsExposedByJS temp = {
      ...
      ExecJSService,         ExecJSResult,       ExecTimeCallback,   ExecJS,
...
std::unique_ptr<WeexJSResult> ScriptBridgeInMultiSo::ExecJSResult(const char *script, long callbackId) {
  return Instance()->script_side()->ExecJsResult(script, callbackId);
}
