        ...
        namespace js {
            ...
            void ScriptBridgeInMultiProcess::RegisterIPCCallback(IPCHandler *handler) {
                ...
                handler->registerHandler(static_cast<uint32_t>(IPCJSMsg::EXECJSSERVICE),
                                         ExecJSService);
                handler->registerHandler(static_cast<uint32_t>(IPCJSMsg::EXECJSRESULT),
                                         ExecJSResult);
            ...
            std::unique_ptr<IPCResult> ScriptBridgeInMultiProcess::ExecJSResult(
                    IPCArguments *arguments) {
                LOGE("--------------------------ScriptBridgeInMultiProcess::ExecJSResult");
               /* const char *source = GetUTF8StringFromIPCArg(arguments, 0);
                return createInt32Result(Instance()->script_side()->ExecJsService(source));*/
                const char *source = GetUTF8StringFromIPCArg(arguments, 0);
                long id = arguments->get<int64_t>(1);
                const std::unique_ptr<WeexJSResult> &ptr = Instance()->script_side()->ExecJsResult(source,id);

                LOGE("----------------");
                LOGE(ptr->data.get() == NULL? "a":"b");
                LOGE("---------------- %s", (*createByteArrayResult(ptr->data.get(), ptr->length)).getByteArrayContent());
