...
std::unique_ptr<WeexJSResult> ScriptSideInMultiProcess::ExecJsResult(const char *script, long callbackId) {
  std::unique_ptr<WeexJSResult> ret;
  try {
    if(sender_ == nullptr) {
      LOGE("ExecJsResult sender is null");
      return NULL;
    }
    std::unique_ptr<IPCSerializer> serializer(createIPCSerializer());
    serializer->setMsg(static_cast<uint32_t>(IPCJSMsg::EXECJSRESULT));
    serializer->add(script, strlen(script));
    serializer->add(static_cast<int64_t>(callbackId));
    std::unique_ptr<IPCBuffer> buffer = serializer->finish();
    std::unique_ptr<IPCResult> result = sender_->send(buffer.get());
    if (result->getType() != IPCType::BYTEARRAY) {
      return ret;
    ...
    memcpy(string, result->getByteArrayContent(), result->getByteArrayLength());
    string[ret->length] = '\0';
    LOGE("－－－－－－－－－－－－－－－ script side in multi process:  %s", string);
