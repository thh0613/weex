...
std::unique_ptr<WeexJSResult> ScriptSideInMultiSo::ExecJsResult(const char *script, long callbackId) {
  if(script_side_functions_ == nullptr) {
    LOGE("ScriptSideInMultiSo::ExecJsResult script_side_functions_ is null");
    std::unique_ptr<WeexJSResult> result;
    return  result;
  }
  return script_side_functions_->funcExeJsResult(script, callbackId);
}
