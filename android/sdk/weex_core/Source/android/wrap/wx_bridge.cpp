...
static jstring ExecJSResult(JNIEnv* env, jobject object, jstring script, jlong callbackId) {
  if (script == nullptr) return env->NewStringUTF("hello weex");
  ...
  auto result =
          WeexCoreManager::Instance()
                  ->getPlatformBridge()
                  ->core_side()
                  ->ExecJsResult(source.getChars(), callbackId);
