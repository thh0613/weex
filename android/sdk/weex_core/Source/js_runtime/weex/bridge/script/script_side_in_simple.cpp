...
std::unique_ptr<WeexJSResult> ScriptSideInSimple::ExecJsResult(const char *script, long callbackId) {
  LOGD("ScriptSideInSimple::ExecJsResult");
  return runtime_->exeJsResult(std::string(script), callbackId);
}
