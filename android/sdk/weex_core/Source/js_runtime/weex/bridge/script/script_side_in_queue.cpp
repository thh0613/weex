...
std::unique_ptr<WeexJSResult> ScriptSideInQueue::ExecJsResult(const char *source, long callbackId) {
  LOGE("--------------ScriptSideInQueue::ExecJsResult");
  ExeJsOnInstanceTask *task = new ExeJsOnInstanceTask(source, callbackId);
  weexTaskQueue_->addTask(task);
  auto future = std::unique_ptr<WeexTask::Future>(new WeexTask::Future());
  task->set_future(future.get());

  LOGE("--------------ScriptSideInQueue  2::ExecJsResult");
