...
public class WXWebView implements IWebView {
    ...
    protected SoftReference<WebView> mWebViewSoftReference;
    ...
    public View getView() {
        ...
        WebView mWebView = new WebView(mContext);//mContext.getApplicationContext();
        mWebViewSoftReference = new SoftReference<>(mWebView);
    ...
    private void showWebView(boolean shown) {
        if (getWebView() != null) {
            getWebView().setVisibility(shown ? View.VISIBLE : View.INVISIBLE);
            if (shown) progressiveShow();
        }
    }

    protected void progressiveShow() {
    ...
    WebView getWebView() {
        ...
        if (mWebViewSoftReference != null && mWebViewSoftReference.get() != null) {
            return mWebViewSoftReference.get();
        }
        return null;
    }
    ...
    private void evaluateJS(String jsStr) {
        if (getWebView() != null) {
            if (SDK_VERSION < 19) {
                getWebView().loadUrl(jsStr);
            } else {
                getWebView().evaluateJavascript(jsStr, null);
            }
        }
    }
